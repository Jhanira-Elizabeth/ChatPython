...
#     def buscar_lugares_cercanos(self, latitude, longitude, max_results=5):
#         """Devuelve los lugares turísticos más cercanos a la ubicación dada (lat, lon).
#         Requiere que cada lugar tenga lat/lon en self.lugares_info.
#         Si no hay info de coordenadas, devuelve una lista vacía."""
#         import math
#         if not hasattr(self, 'lugares_info') or not self.lugares:
#             return []
#         lugares_cercanos = []
#         for lugar in self.lugares:
#             info = self.lugares_info.get(lugar)
...
#         return lugares_cercanos[:max_results]
#     def __init__(self, filepath='turismo_data_completo_v2.jsonl'):
#         self.lugares_info = {} # nombre_lugar -> {'latitude':..., 'longitude':...}
...
#         # Si el item tiene coordenadas, guárdalas en lugares_info
#         if 'messages' in item and 'location' in item:
#             loc = item['location']
#             lugar_nombre = loc.get('nombre_lugar')
#             lat = loc.get('latitude')
#             lon = loc.get('longitude')
#             if lugar_nombre and lat and lon:
#                 self.lugares_info[lugar_nombre] = {'latitude': lat, 'longitude': lon}
...
#         # Loguear y buscar lugares cercanos si se recibe ubicación
#         lugares_cercanos = None
#         if latitude is not None and longitude is not None:
#             logger.info(f"Ubicación recibida: lat={latitude}, lon={longitude}")
#             lugares_cercanos = knowledge_base.buscar_lugares_cercanos(latitude, longitude)
...
#             # else: Si la búsqueda en internet no dio resultados, se mantiene la respuesta local

#         # Si tienes lugares cercanos, añádelos a la respuesta
#         if lugares_cercanos:
#             respuesta_estructurada['lugares_cercanos'] = lugares_cercanos

#         return jsonify({
#             'response': respuesta_estructurada,
#             'status': 'success',
#             'fuente': fuente,
